...
def get_history_stats(_db_conn, teamA, teamB, all_comps, all_season_schemas, side):
    sql_file = read_sql_file(
        "components/queries/team_stats/all_competitions/head_to_head_history/get_head_to_head_history_stats.sql",
        teamA=teamA,
        teamB=teamB,
...
def get_history_matches(_db_conn, teamA, teamB, all_comps, all_season_schemas, side):
    sql_file = read_sql_file(
        "components/queries/team_stats/all_competitions/head_to_head_history/get_head_to_head_history_matches.sql",
        teamA=teamA,
        teamB=teamB,
        comps=', '.join([f"'{comp}'" for comp in all_comps]),
        seasons=', '.join([f"'{season_schema[7:]}'" for season_schema in all_season_schemas]),
...
def get_head_to_head_history(db_conn):
    prefix = "head_to_head_history"
    ...
    if teamA and teamB and teamA != teamB:
        ...
        df = get_history_stats(db_conn, teamA, teamB, all_comps, all_season_schemas, side)
